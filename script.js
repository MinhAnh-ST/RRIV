...
async function fetchRealDataFromGateway() {
  try {
    const response = await fetch(`${GATEWAY_URL}/api/devices`);
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const devices = await response.json();

    stations = devices.map((dev, idx) => {
      const dataArr = dev.data;
    



      const realLat = parseFloat(dev.lat);
      const realLng = parseFloat(dev.lng);
      const hasGPS = realLat !== 0 && realLng !== 0 && !isNaN(realLat);

      return {
        id: dev.id,
        name: dev.name || `Trạm ${idx+1}`,
        isNDVI: dev.isNDVI || false,
        status: dev.status !== undefined ? dev.status : true,
        data: [dataArr[0], dataArr[1], dataArr[2], dataArr[3], dataArr[4], dataArr[5], dataArr[6], dataArr[7], dataArr[8], dataArr[9]],
        location: {
          lat: hasGPS ? realLat : (10.8231 + (idx * 0.001)),
          lng: hasGPS ? realLng : (106.6297 + (idx * 0.001)),
          address: hasGPS ? `Vị trí thực tế trạm ${idx+1}` : `Vị trí giả lập trạm ${idx+1}`,
          isRealGPS: hasGPS
        }
      };
    });
...
async function fetchNDVIFromGateway() {
  try {
    const res = await fetch(`${GATEWAY_URL}/api/ndvi`);
