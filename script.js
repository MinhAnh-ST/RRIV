...
function renderStationGrid() {
  ...
  if (!document.getElementById('sgrid-style')) {
    ...
    s.textContent = `
      ...
      #stationGrid { display:grid; grid-template-columns:1fr; gap:10px; }
      ...
      .sg-badge { font-size:10px; padding:2px 7px; border-radius:10px; font-weight:500; }
      .sg-table { width:100%; border-collapse:collapse; font-size:11px; }
      .sg-table th { background:#f8f9fa; color:#6c757d; font-weight:600;
        font-size:10px; padding:5px 6px; text-align:center;
        border-bottom:1px solid #e9ecef; letter-spacing:.2px; }
      .sg-table td { padding:5px 6px; text-align:center; border-bottom:1px solid #f5f5f5; }
      .sg-table tr:last-child td { border-bottom:none; }
      .sg-td-sensor      { font-weight:600; font-size:12px; color:#212529; }
      .sg-td-sensor-ok   { font-weight:600; font-size:12px; color:#27a745; }
      .sg-td-sensor-warn { font-weight:600; font-size:12px; color:#dc3545; }
      .sg-td-calc        { font-weight:600; font-size:12px; color:#B8860B; }
      .sg-td-calc-warn   { font-weight:600; font-size:12px; color:#dc3545; }
      .sg-td-manual      { font-weight:500; font-size:11px; color:#534AB7; }
      .sg-td-manual-warn { font-weight:500; font-size:11px; color:#e74c3c; }
      .sg-td-na          { color:#ced4da; font-size:10px; }
      .sg-legend { display:flex; gap:10px; padding:6px 12px;
        border-top:1px solid #f5f5f5; flex-wrap:wrap; }
      .sg-leg-item { display:flex; align-items:center; gap:4px; font-size:10px; color:#6c757d; }
      .sg-leg-dot { width:8px; height:8px; border-radius:2px; }
      .sg-footer { padding:7px 12px; display:flex; align-items:center;
        justify-content:space-between; border-top:1px solid #f5f5f5; flex-wrap:wrap; gap:4px; }
      .sg-bat-shell { width:26px; height:11px; border:1.5px solid #adb5bd; border-radius:2px;
        padding:1px; display:inline-flex; align-items:center; }
      .sg-bat-inner { height:100%; border-radius:1px; }
      .sg-no-signal { padding:18px 12px; text-align:center; flex:1;
        display:flex; flex-direction:column; align-items:center; justify-content:center; gap:6px; }
    `;
    document.head.appendChild(s);
  }

  // ── Lấy manual data mới nhất cho từng trạm ──
  function getManualLatest() {
    try {
      const saved = localStorage.getItem('rriv_manual_latest');
      return saved ? JSON.parse(saved) : {};
    } catch(e) { return {}; }
  }

  // ── Tính N, P, K, CEC từ công thức ──
  function calcNPK(ph, ec, temp, ndvi, spad, p_manual, k_manual, texture, c_org) {
    // Hệ số tạm thời, có thể cấu hình sau
    const ndviVal = (typeof ndvi === 'number' && !isNaN(ndvi)) ? ndvi : 0.3;
    const spadVal = (typeof spad === 'number' && !isNaN(spad)) ? spad : 40;
    const textureVal = (typeof texture === 'number' && !isNaN(texture)) ? texture : 20;
    const cVal = (typeof c_org === 'number' && !isNaN(c_org)) ? c_org : 2;

    // N = 2.5×pH + 0.8×T + 1.2×EC + 15×NDVI + 0.3×SPAD - 30
    const N = +(2.5*ph + 0.8*temp + 1.2*ec + 15*ndviVal + 0.3*spadVal - 30).toFixed(1);

    // P = 1.5×pH + 0.5×SPAD + 8×NDVI - 10
    const P = +(1.5*ph + 0.5*spadVal + 8*ndviVal - 10).toFixed(1);

    // K = 0.3×EC + 0.4×pH + 6×NDVI
    const K = +(0.3*ec + 0.4*ph + 6*ndviVal).toFixed(2);

    // CEC = 0.8×pH + 1.5×EC + 0.2×Texture + 2×C_org
    const CEC = +(0.8*ph + 1.5*ec + 0.2*textureVal + 2*cVal).toFixed(1);

    return { N: Math.max(0,N), P: Math.max(0,P), K: Math.max(0,K), CEC: Math.max(0,CEC) };
  }

  function textureLabel(pct) {
    if (!pct && pct !== 0) return '—';
    if (pct < 20) return `${pct}% (Cát)`;
    if (pct < 40) return `${pct}% (Thịt)`;
    return `${pct}% (Sét)`;
  }
  ...
  function isAdmin() {
    return typeof currentUser !== 'undefined' && currentUser &&
      (currentUser.role === 'admin' || currentUser.stations === 'all');
  }

  // ── Header tổng quan ──
  const totalOnline  = stations.filter(s => s.status && s.data && (s.data[0]||s.data[1]||s.data[3])).length;
  ...
  if (typeof lastNDVIData !== 'undefined' && lastNDVIData && lastNDVIData.valid) {
    const r = lastNDVIData.S2_411.red, n = lastNDVIData.S2_411.nir;
    if (r + n > 0.001) ndviGlobal = (n - r) / ((n + r) || 0.001);
  }

  // ── Ngưỡng ──
  const st = settings.stations;
  const THRESH = {
    pH:   { min: st.phMin,   max: st.phMax   },
    VWC:  { min: st.humMin,  max: st.humMax  },
    EC:   { min: st.ecMin,   max: st.ecMax   },
    T:    { min: st.tempMin, max: st.tempMax },
    N:    { min: 20,  max: 80  },
    P:    { min: 5,   max: 25  },
    K:    { min: 0.2, max: 1.5 },
    Ca:   { min: 2,   max: 10  },
  ...
  function tdCls(val, key, type) {
    if (val === null || val === undefined || isNaN(parseFloat(val))) return 'sg-td-na';
    const t = THRESH[key];
    const out = t ? (parseFloat(val) < t.min || parseFloat(val) > t.max) : false;
    if (type === 'manual') return out ? 'sg-td-manual-warn' : 'sg-td-manual';
    if (type === 'calc')   return out ? 'sg-td-calc-warn'   : 'sg-td-calc';
    return out ? 'sg-td-sensor-warn' : 'sg-td-sensor-ok';
  }
  ...
  // ── Render từng card ──
  grid.innerHTML = '';
  const manualLatest = getManualLatest();
  ...
  stations.filter(s => !s.isNDVI).slice(0, 6).forEach((station) => {
    ...
    const bat      = hasData ? parseFloat(station.data[7]) : 0;
    const batCol   = batColor(bat);

    let badgeHtml;
    if (isOffline) badgeHtml = `<span class="sg-badge" style="background:#f8f9fa;color:#6c757d;">CHƯA CÓ TÍN HIỆU</span>`;
    else if (isAlert) badgeHtml = `<span class="sg-badge" style="background:#fff3cd;color:#856404;">⚠ CẢNH BÁO</span>`;
    else badgeHtml = `<span class="sg-badge" style="background:#d4edda;color:#155724;">ONLINE</span>`;
      ...
      grid.innerHTML += `
        <div class="sg-card">
          ...
          <div class="sg-no-signal">
            <i class="fas fa-satellite-dish" style="font-size:26px;color:#dee2e6;"></i>
    ...
    // Data thủ công (lấy từ localStorage — mẫu gần nhất)
    const m = manualLatest[station.name] || manualLatest['all'] || {};
    const spad    = parseFloat(m.SPAD)    || null;
    const c_org   = parseFloat(m.humus)   || null;
    const texture = parseFloat(m.Texture) || null;
    const ca      = parseFloat(m.Ca)      || null;
    const mg      = parseFloat(m.Mg)      || null;
    const cu      = parseFloat(m.Cu)      || null;
    const zn      = parseFloat(m.Zn)      || null;
    const mn      = parseFloat(m.Mn)      || null;

    // Tính N, P, K, CEC
    const calc = calcNPK(ph, ec, temp, ndviGlobal, spad, null, null, texture, c_org);

    // Hiện SPAD/manual chỉ khi là admin
    const showManual = isAdmin();
    ...
    grid.innerHTML += `
      <div class="sg-card${isAlert ? ' sg-alert' : ''}">
        ...
        <div style="overflow-x:auto;">
          ...
          <thead>
          <tr>
            <th title="pH đất" style="color:#212529">pH</th>
            <th title="Hàm lượng nước thể tích (%)" style="color:#212529">VWC<br><span style="font-size:9px;font-weight:400">%</span></th>
            <th title="Độ dẫn điện (dS/m)" style="color:#212529">EC<br><span style="font-size:9px;font-weight:400">dS/m</span></th>
            <th title="Nhiệt độ đất (°C)" style="color:#212529">T<br><span style="font-size:9px;font-weight:400">°C</span></th>
            <th title="SPAD/CCI - nhập thủ công" style="color:#534AB7">SPAD<br><span style="font-size:9px;font-weight:400">CCI</span></th>
            <th title="Hàm lượng mùn hữu cơ (%)" style="color:#534AB7">C<br><span style="font-size:9px;font-weight:400">%</span></th>
            <th title="Dung tích hấp thụ cation (tính toán)" style="color:#B8860B">CEC<br><span style="font-size:9px;font-weight:400">meq</span></th>
            <th title="Thành phần cơ giới đất" style="color:#534AB7">Tex<br><span style="font-size:9px;font-weight:400">%sét</span></th>
            <th title="Đạm tổng số (tính toán)" style="color:#B8860B">N<br><span style="font-size:9px;font-weight:400">mg/kg</span></th>
            <th title="Lân dễ tiêu (tính toán)" style="color:#B8860B">P<br><span style="font-size:9px;font-weight:400">mg/kg</span></th>
            <th title="Kali trao đổi (tính toán)" style="color:#B8860B">K<br><span style="font-size:9px;font-weight:400">meq</span></th>
            <th title="Canxi - nhập thủ công" style="color:#534AB7">Ca<br><span style="font-size:9px;font-weight:400">meq</span></th>
            <th title="Magie - nhập thủ công" style="color:#534AB7">Mg<br><span style="font-size:9px;font-weight:400">meq</span></th>
            <th title="Đồng - nhập thủ công" style="color:#534AB7">Cu<br><span style="font-size:9px;font-weight:400">mg/kg</span></th>
            <th title="Kẽm - nhập thủ công" style="color:#534AB7">Zn<br><span style="font-size:9px;font-weight:400">mg/kg</span></th>
            <th title="Mangan - nhập thủ công" style="color:#534AB7">Mn<br><span style="font-size:9px;font-weight:400">mg/kg</span></th>
          </tr>
        </thead>
            <tbody>
              <tr>
                <td class="${tdCls(ph,'pH','sensor')}">${fmt(ph,1)}</td>
                <td class="${tdCls(vwc,'VWC','sensor')}">${fmt(vwc,1)}</td>
                <td class="${tdCls(ec,'EC','sensor')}">${fmt(ec,2)}</td>
                <td class="${tdCls(temp,'T','sensor')}">${fmt(temp,1)}</td>
                <td class="${showManual ? tdCls(spad,'SPAD','manual') : 'sg-td-na'}">
                  ${showManual ? fmt(spad,0) : '🔒'}
                </td>
                <td class="${showManual ? tdCls(c_org,'C','manual') : 'sg-td-na'}">
                  ${showManual ? fmt(c_org,1) : '🔒'}
                </td>
                <td class="${tdCls(calc.CEC,'CEC','calc')}">${fmt(calc.CEC,1)}</td>
                <td class="${showManual ? (texture!==null?'sg-td-manual':'sg-td-na') : 'sg-td-na'}">
                  ${showManual ? (texture!==null ? textureLabel(texture) : '—') : '🔒'}
                </td>
                <td class="${tdCls(calc.N,'N','calc')}">${fmt(calc.N,1)}</td>
                <td class="${tdCls(calc.P,'P','calc')}">${fmt(calc.P,1)}</td>
                <td class="${tdCls(calc.K,'K','calc')}">${fmt(calc.K,2)}</td>
                <td class="${showManual ? tdCls(ca,'Ca','manual') : 'sg-td-na'}">
                  ${showManual ? fmt(ca,1) : '🔒'}
                </td>
                <td class="${showManual ? tdCls(mg,'Mg','manual') : 'sg-td-na'}">
                  ${showManual ? fmt(mg,1) : '🔒'}
                </td>
                <td class="${showManual ? tdCls(cu,'Cu','manual') : 'sg-td-na'}">
                  ${showManual ? fmt(cu,1) : '🔒'}
                </td>
                <td class="${showManual ? tdCls(zn,'Zn','manual') : 'sg-td-na'}">
                  ${showManual ? fmt(zn,1) : '🔒'}
                </td>
                <td class="${showManual ? tdCls(mn,'Mn','manual') : 'sg-td-na'}">
                  ${showManual ? fmt(mn,1) : '🔒'}
                </td>
              </tr>
            </tbody>
        ...
        <!-- Legend -->
        <div class="sg-legend">
          <div class="sg-leg-item"><div class="sg-leg-dot" style="background:#212529"></div>Cảm biến</div>
          <div class="sg-leg-item"><div class="sg-leg-dot" style="background:#B8860B"></div>Tính toán</div>
          ${showManual ? `<div class="sg-leg-item"><div class="sg-leg-dot" style="background:#534AB7"></div>Thủ công</div>` : ''}
          ${!showManual ? `<div class="sg-leg-item" style="color:#adb5bd"><i class="fas fa-lock" style="font-size:9px"></i> Cần quyền Admin</div>` : ''}
          ${alerts.length > 0 ? `<div class="sg-leg-item" style="color:#dc3545">⚠ ${alerts[0].text}</div>` : ''}
        </div>
        ...
        <div class="sg-footer">
          <div style="display:flex;align-items:center;gap:4px;">
            <div class="sg-bat-shell">
              <div class="sg-bat-inner" style="width:${Math.min(100,bat)}%;background:${batCol};"></div>
            </div>
            <span style="font-size:11px;font-weight:500;color:${batCol};">${bat.toFixed(0)}%</span>
            <span style="font-size:10px;color:#adb5bd;margin-left:2px;">
              ${station.data[8]===1?'⚡':station.data[8]===2?'✅':'🔋'}
            </span>
          </div>
          <div style="font-size:10px;color:#ced4da;">
  ...
  // ── Cập nhật sensor 411/412 header ──
  if (lastNDVIData && lastNDVIData.valid) {
    const s = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
    s('411-red',  lastNDVIData.S2_411.red.toFixed(4));
    s('411-nir',  lastNDVIData.S2_411.nir.toFixed(4));
    s('411-tilt', lastNDVIData.S2_411.angle.toFixed(1));
    s('412-red',  lastNDVIData.S2_412.red.toFixed(4));
    s('412-nir',  lastNDVIData.S2_412.nir.toFixed(4));
    s('412-tilt', lastNDVIData.S2_412.angle.toFixed(1));
  }
...
/*const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    try {
      const reply = await aiCallAPI(aiChatHistory, 'Ban la chuyen gia nong nghiep AI MIA. Phan tich du lieu cam bien va dua ra khuyen nghi cu the. Tra loi bang tieng Viet.');
      document.getElementById('aiTyping')?.remove();
      aiChatHistory.push({ role: 'assistant', content: reply });
      appendAIMsg('assistant', reply);
    } catch(err) {
...
async function aixSend() {
  ...
  try {
   ...
   /* const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    try {
      const reply = await aiCallAPI(aixHistory, systemPrompt);
      aixHideTyping();
      aixHistory.push({ role: 'assistant', content: reply });
      aixAppendMsg('assistant', reply);
      const cfg = aiKeyLoad();
      aixProvider = cfg.provider || 'gemini';
      aixUpdateBadge(aixProvider);
    } catch(err) {
      aixHideTyping();
      aixAppendMsg('assistant', '❌ ' + err.message);
    }
...
async function mdLoadFields() {
  ...
  if (manualFields.length === 0) {
    manualFields = [
      ...
      {id:'humus',  name:'Hàm lượng mùn (C)',    unit:'%',        ref_min:1,    ref_max:5,    required:false},
      {id:'SPAD',   name:'SPAD/CCI',             unit:'',         ref_min:30,   ref_max:60,   required:false},
      {id:'Texture',name:'Texture (% sét)',      unit:'%',        ref_min:0,    ref_max:100,  required:false},
      {id:'CEC',    name:'CEC',                  unit:'meq/100g', ref_min:5,    ref_max:40,   required:false},
      {id:'Cu',     name:'Đồng (Cu)',            unit:'mg/kg',    ref_min:0.2,  ref_max:20,   required:false},
      {id:'Zn',     name:'Kẽm (Zn)',             unit:'mg/kg',    ref_min:0.5,  ref_max:5,    required:false},
      {id:'Mn',     name:'Mangan (Mn)',          unit:'mg/kg',    ref_min:5,    ref_max:50,   required:false},
    ];
...
<div id="md-entry" class="md-sec show">
  <div class="md-card">
    ...
    <div class="row mb-3 pb-3 border-bottom">
      ...
      <div class="col-md-4 mb-2">
        <label class="form-label" style="font-size:12px;color:#6c757d">Trạm áp dụng</label>
        <select class="form-select form-select-sm" id="md-station">
          <option value="all">Tất cả trạm</option>
          <option value="Trạm 1">Trạm 1</option>
          <option value="Trạm 2">Trạm 2</option>
          <option value="Trạm 3">Trạm 3</option>
          <option value="Trạm 4">Trạm 4</option>
          <option value="Trạm 5">Trạm 5</option>
          <option value="Trạm 6">Trạm 6</option>
        </select>
      </div>
      <div class="col-md-4 mb-2">
...
async function mdSave() {
  ...
  try {
    ...
    const station = document.getElementById('md-station')?.value || 'all';
  ...
  body: JSON.stringify({
    ...
    if (res.ok) {
      ...
      // Lưu vào localStorage để trang Giám sát đọc được
      const latestCache = JSON.parse(localStorage.getItem('rriv_manual_latest') || '{}');
      if (station === 'all') {
        // Áp dụng cho tất cả trạm
        ['Trạm 1','Trạm 2','Trạm 3','Trạm 4','Trạm 5','Trạm 6'].forEach(t => {
          latestCache[t] = values;
        });
      } else {
        latestCache[station] = values;
      }
      localStorage.setItem('rriv_manual_latest', JSON.stringify(latestCache));
...
// ================================================================
// AI KEY MANAGER — Quản lý API key AI qua giao diện
// ================================================================

function aiKeyLoad() {
  return JSON.parse(localStorage.getItem('rriv_ai_config') || '{}');
}
...
function aiKeySave() {
  ...
  localStorage.setItem('rriv_ai_config', JSON.stringify(config));
  const st = document.getElementById('ai-key-status');
  if (st) { st.style.color='#3B6D11'; st.textContent='✅ Đã lưu!'; setTimeout(()=>st.textContent='',3000); }
  showToast('✅ Đã lưu cấu hình AI','success');
}

function aiKeyToggleUI() {
  const provider = document.getElementById('ai-provider')?.value;
  const labelEl  = document.getElementById('ai-key-label');
  const subEl    = document.getElementById('ai-key-sub');
  const noteRow  = document.getElementById('gemini-note-row');
  if (provider === 'gemini') {
    if (labelEl) labelEl.textContent = 'Gemini API Key';
    if (subEl)   subEl.textContent   = 'Lấy miễn phí tại aistudio.google.com/apikey';
    if (noteRow) noteRow.style.display = '';
  } else {
    if (labelEl) labelEl.textContent = 'Groq API Key';
    if (subEl)   subEl.textContent   = 'Lấy tại console.groq.com — miễn phí có giới hạn req/phút';
    if (noteRow) noteRow.style.display = 'none';
  }
}

async function aiKeyTest() {
  const provider = document.getElementById('ai-provider')?.value || 'gemini';
  const key      = document.getElementById('ai-api-key')?.value?.trim() || aiKeyLoad().key || '';
  const st       = document.getElementById('ai-key-status');
  if (!key) { if(st){st.style.color='#dc3545';st.textContent='❌ Chưa nhập key';} return; }
  if (st) { st.style.color='#6c757d'; st.textContent='⏳ Đang test...'; }
  try {
    let ok = false;
    if (provider === 'gemini') {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${key}`,{
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({contents:[{parts:[{text:'Xin chào, bạn có hoạt động không?'}]}]})
      });
      ok = res.ok;
    } else {
      const res = await fetch('https://api.groq.com/openai/v1/chat/completions',{
        ...
        body: JSON.stringify({model:'llama-3.1-8b-instant',max_tokens:10,messages:[{role:'user',content:'hi'}]})
      });
      ok = res.ok;
    }
    if (st) { st.style.color = ok?'#3B6D11':'#dc3545'; st.textContent = ok?'✅ Kết nối OK!':'❌ Key không hợp lệ'; }
  } catch(e) {
    if (st) { st.style.color='#dc3545'; st.textContent='❌ Lỗi mạng'; }
  }
}

// Load config khi mở settings
const _origShowSettingsForAI = window.showSettings;
window.showSettings = function() {
  if (_origShowSettingsForAI) _origShowSettingsForAI.apply(this, arguments);
  setTimeout(() => {
    const cfg = aiKeyLoad();
...
// ── Hàm gọi AI thống nhất (dùng cho cả Chat và Expert) ──
async function aiCallAPI(messages, systemPrompt) {
  ...
  if (!key) {
    return '❌ Chưa cấu hình API key. Vào **Cài đặt → API Key AI** để nhập key Gemini hoặc Groq.';
  }

  if (provider === 'gemini') {
    // Build contents cho Gemini
    ...
    if (!res.ok) {
      const err = await res.json().catch(()=>({}));
      throw new Error(`Gemini ${res.status}: ${err.error?.message||'Lỗi không xác định'}`);
    }
    const data = await res.json();
    ...
    if (!res.ok) {
      const err = await res.json().catch(()=>({}));
      throw new Error(`Groq ${res.status}: ${err.error?.message||'Lỗi không xác định'}`);
    }
    const data = await res.json();
...
// ── Load manual data mới nhất từ Sheets vào localStorage ──
async function mdSyncLatestToCache() {
  try {
    const res  = await fetch(`${SHEETS_URL}?action=getManual&days=30`);
    const rows = await res.json();
    if (!rows || rows.length === 0) return;

    // Lấy dòng mới nhất (cuối mảng)
    const latest = rows[rows.length - 1];
    const values = {};
    const fieldKeys = ['N','P','K','Ca','Mg','humus','SPAD','Texture','CEC','Cu','Zn','Mn','S','D'];
    fieldKeys.forEach(k => {
      if (latest[k] !== undefined && latest[k] !== '') {
        values[k] = parseFloat(latest[k]);
      }
    });

    const latestCache = JSON.parse(localStorage.getItem('rriv_manual_latest') || '{}');
    latestCache['all'] = values;
    latestCache[latest['Ten mau'] || 'all'] = values;
    localStorage.setItem('rriv_manual_latest', JSON.stringify(latestCache));
    console.log('[mdSync] Cache updated:', values);
  } catch(e) {
    console.warn('[mdSync] Lỗi:', e);
  }
}
